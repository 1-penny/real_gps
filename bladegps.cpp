...
void* tx_task(void* arg)
{
	...
	while (1) {
		...
		s->tx.dev2.send(s->tx.buffer, SAMPLES_PER_BUFFER, TIMEOUT_MS);
		if (is_fifo_write_ready(s)) {
			/*
			printf("\rTime = %4.1f", s->time);
			s->time += 0.1;
...
int main(int argc, char* argv[])
{
	...
	s.status = s.tx.dev2.config(xb_board);
	if (s.status != 0) {
		goto out;
	}
	...
	s.status = s.tx.dev2.enable(true);
	if (s.status != 0) {
	...
	s.status = s.tx.dev2.enable(false);
	if (s.status != 0)
